...
object AppUpdateManager {
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                for (path in pathsToTry) {
                    try {
                        val url = "${com.example.util.SecureConfig.firebaseDatabaseUrl}$path"
                ...
                if (targetVersionCode == -1) {
                    val fallbackUrl = "${com.example.util.SecureConfig.firebaseDatabaseUrl}versionId.json"
    ...
    suspend fun publishUpdateConfig(versionId: Int, apkFileId: String?): Boolean = withContext(Dispatchers.IO) {
        try {
            val url = "${com.example.util.SecureConfig.firebaseDatabaseUrl}update_config.json"
            val json = JSONObject()
